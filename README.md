...
> **備份那一步需要密碼，而控制台不收密碼**：從頁面跑 `update` 時，若沒有事先設好
> 環境變數 `TWCRAWL_BACKUP_PASSWORD`，備份會以人話跳過（其餘步驟照跑）。要產生
> 備份包就在終端機跑 `twcrawl backup`，或先設好那個環境變數再啟動控制台。
...
```powershell
python tests\test_twcrawl.py                    # 65 個測試，不需要 pytest
...
控制台的長工路徑另有三支：登入交接（子行程印出的標記 → 按鈕 → 訊號檔 → 子行程繼續，兩端同一份定義）、中止（真的造一棵行程樹，確認孫行程也被收掉——只殺父的話留下的就是還開著的 Chromium）、以及工作端點本身（真的起子行程跑 `export` 與 `fetch`，順帶釘住白名單、參數形狀與跨來源防護）。

...
- ⬜（可選）statusCodes 狀態碼翻中文（實測沒有那份字典，見上）；FDA 開放資料的
  「回收藥品資料集」可另接（食品類無資料集）
- ⬜ **價格追蹤（同店限定）＝go，但還沒開票**（2026-08-01 品名量測 issue #9 裁決；
  統計全文在該票留言）。「等品名正規化有解」這個前提**已被推翻**——同店的品名
  原文完全穩定（normalize 零合併），原文精確比對就有 138 個可追蹤配對、其中
  63 個有 ≥3 個價格點。**跨店比價才需要正規化，那條確定 no-go；品項層級分類
  也是 no-go**（58.7% 的列是餐飲菜名、長尾 67.9% 只出現一次，維持 item_rules
  逐案擴充）。動工前先開 issue 寫 AC（repo 慣例）
- ⬜（可選）儀表板分類顏色跨次匯出的穩定對應

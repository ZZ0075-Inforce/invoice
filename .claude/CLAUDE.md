...
python tests/test_twcrawl.py          # 測試（71/71，不用 pytest）
...
│                 在不同發票得到不同分類）。`stats()`（全庫筆數＋max(inv_date)；
│                 update 組步驟與 restore 的還原後驗證都要，收進來順帶讓
│                 commands.py 完全不含 SQL）。**一次性的聚合查詢留在原地**
│                 （FDA 來源統計、常買品項 top3）——搬進來只是換個地方放 SQL
...
│                 偵測也在這——月報磚與查詢頁視圖同源；`_detect_price` 同店價格
│                 追蹤（issue #23）也是同一個形狀（吃 invoice_rows 的純函式）：
│                 配對鍵（顯示店家名, 品名原文）不正規化——issue #9 量測顯示同店
│                 原文已完全穩定；取價 `_price_of` **單價優先、只有整個缺了才用
│                 金額÷數量回推**（8.1% 的列數量不是 1，拿 amount 比價會把「買
│                 兩包」讀成漲一倍），非正價（贈品、折讓）不採用也不回推。
│                 「漲了」＝最近一次 > 先前價格中位數 ×(1+risePct%)，不用首末比
│                 （中間漲上去又跌回來會被讀成沒漲，實測 9 個配對是這型）。
│                 三道清理都留數字在 `stats`：非正價的列、同日同名不同價的那天、
│                 高低比超過 spreadCap 的配對（**只擋漲價清單、全表照留**——庫內
│                 就有一個 6.5 倍的假貨會排在漲幅第一列）。門檻 PRICE_RULE 隨
│                 payload 走，同 FIXED_RULE——`minPoints` **刻意不夾下限**
│                 （查詢頁直接印這個數字，夾住就會頁面說 1 而行為是 2；
│                 下限 2 由測試釘住，設 1 也真的會炸：中位數取 series[:-1]）；
│                 load_budget 讀
...
│                 一個工作＝**依序跑的幾道指令**（`Steps`；#22 的 import＝匯入
│                 →重生報表）。這層刻意不交給頁面串：頁面在工作結束後再送一次
...
│                 檔案總管「複製檔案路徑」附帶的引號；檔案本身合不合法交給指令
│                 端講人話，不抄第二份訊息。
...
│                 `existing_data` 由 `_targets()` 導出**不另外列一份**（手打第二
│                 份的話，備份日後多收一個東西這裡會靜默漏掉，而漏掉的後果正好
│                 是默默蓋掉使用者資料）。路徑字面一律問 Workspace（`ws.state.name`）
...
├── web/query.html 查詢頁四視圖（發票清單搜尋/篩選/逐張展開品項、店家查詢、
│                 固定支出偵測——同店家＋金額相近 ±max(15,5%)＋週期 25–400 天
│                 ＋至少 3 次、價格追蹤）；
│                 深連結參數 ?seller/?cat/?from/?to/?unnecessary/?q/?view；
│                 店家查詢的排行/下拉由發票聚合而非 sellers[].category——
│                 品項覆寫後店家可跨分類，發票聚合才與磚的金額天生一致。
│                 價格追蹤（#23）：上半漲價清單、下半全表可搜尋，判定與清理全在
│                 export._detect_price，這裡只排版。點列展開單價時間線——**價格
│                 點 ≥3 才畫折線**（兩點的折線是一條直線，畫出來只是假裝有趨勢），
│                 日期那列的發票號碼點下去帶 ?q= 跳發票清單。漲價**不套 .ok**
│                 （那是 --up-good 綠色，會讀成「漲價是好事」）——有事的用一般色、
│                 沒事的靜音，不必再發明色票。**三個排除數都要印**（#24）：
...
│                 店家搜尋——同一個框輸入即過濾圓點（與圖例/時間取交集、
│                 比對顯示名含招牌名）、datalist 選定另外定位開 popup；
│                 popup「查這家」連查詢頁）
├── probe.py      頁面結構偵察
...
- **FDA 開放資料平台沒有食品類的下架／回收資料集**（只有藥品的）——這就是三個
  來源都得爬 ASP.NET 頁面、而不是叫一支開放資料 API 的原因。藥品那份不接：
  比對範圍是食品（見 CONTEXT.md「問題商品」）
...
- **查詢區間有兩條硬規則**（2026-08-01 實測，違反則 `getSearchCarrierInvoiceListJWT`
  回 HTTP 400 `The invoice query interval setting is abnormal.`；不是 401、
  也不是「查無資料」）：①**起始日不得早於某個下限**——實測當日下限恰為
  2026-01-01，早一天（2025-12-31 單日）就被擋；②**區間必須落在同一個曆月內**
  ——近期的跨月區間（如 2026-05-01～08-01）一樣被擋。
  ②正是 `fetch` 逐月呼叫在規則上必要的原因，不只是仿照 UI。
  ①的下限是「當年度」還是「滾動 N 個月」**尚無法判定**——實測當日兩者預測
  完全重合（2026-08-01 減 7 個月＝2026-01-01）；下個月初再測一次即可分辨：
  下限若移到 2026-02-01 就是滾動窗口，仍停在 2026-01-01 就是當年度制
...
- **查詢窗口比文件寫的窄很多，且表頭沒有比明細長**（2026-08-01 實測，見
  「站台實測事實」的查詢區間規則）——舊記載「表頭可回溯前 7 年」不成立。
  平台不是典藏處，本機資料庫才是：**沒定期 `fetch` 就是永久遺失**
- 載具需先歸戶（悠遊卡、一卡通等歸戶到手機條碼才查得到）
...
> **不變式：同一件事不得同時出現在 ⬜ 與 ✅。** 加 ✅ 條目時要回頭刪掉描述
> 同一件事的 ⬜。這兩份清單是同一種知識（「這件事做了沒」）的兩份拷貝，而
> **清單複製第二份**是這個 repo 的頭號紅線，失敗方式一律是靜默的。
> 2026-08-02 清出三條死項：statusCodes 翻中文（#14 早就做完，只是走靜態
> 對照而非那份不存在的字典）、色槽跨次穩定對應（就是 #10）、FDA 回收藥品
> 資料集（探索 FDA 開放資料時順手記的安慰獎，從來不是需求）。
...
- ✅ 地圖店家搜尋（2026-08-01，issue #15）：既有「找店家」框擴充成輸入即
  過濾——關鍵字比對頁面顯示的店家名（含招牌名），render 內與圖例隱藏、
  時間區間**取交集**（不符者同圖例隱藏：不畫也不進無座標清單）；清空即
  恢復；datalist 選定同名店家仍另外定位開 popup。純客戶端、payload 未動
...
- ✅ `twcrawl restore`（2026-08-02，issue #17）：備份從此不再單向。`restore <包>`
  ...
  兩軸 review 後補修六件：①`existing_data` 原本手抄 `_targets()` 的清單，
  改成導出（同一個 commit 一邊宣告不變式一邊破壞它；漏掉的後果正好是默默
  蓋掉使用者資料，已加測試釘住）②「沒有資料庫」原本謊稱「不是 twcrawl
  備份包」——資料庫還不存在時 backup 也會產出合法的包，訊息改講實話
  ③找不到包／密碼錯／不是備份包補上「→ 下一步」④`max(inv_date)` 兩個
  caller 了，收進 `db.stats()`，commands.py 從此不含 SQL ⑤`STATE_DIRNAME`
...
- ✅ `twcrawl import`（2026-08-02，issue #19）：歷年資料唯一入口——平台的查詢
  ...
  給人話且不回放內容）
- ✅ 控制台接上匯入（2026-08-02，issue #22；**「操作更順手」批次 #17–#22
  六張全結**）：控制台貼上 CSV 路徑就匯入，**同一個工作接著重生報表**。
  關鍵決定是那個「接著」放哪一層——交給頁面在工作結束後再送一次請求的話，
  使用者關掉分頁就靜默留下舊報表，而他不會知道自己看到的是舊的；所以
  `jobs` 的一個工作改成**依序跑的幾道指令**（`Steps`），前一段非零就不做下
...
- ✅ 歷年回溯已結案：**做不到，而且不是實作問題**（2026-08-01 使用者拍板要抓、
  當場實測）。平台的查詢區間規則把起始日擋在下限之前（細節見「站台實測事實」），
  2025-12-31 連單日查詢都回 400，所以「更早只抓得到表頭」也是錯的——表頭一樣拿不到。
  同輪把可抓窗口抓滿：2026-01～08 全月重抓含明細，485→496 張、1,231→1,248 列。
  更早的資料只剩官方途徑（檢具身分證、載具影本與申請書向平台專案申請），
  要不要走由使用者決定。**推論：平台不是典藏處，`update` 的每月例行就是典藏本身**
- ✅ **價格追蹤（同店限定）（2026-08-02，issue #23）**：查詢頁第四視圖
  `?view=price`，上半漲價清單、下半全表。前提是 issue #9 的量測推翻了「等品名
  正規化有解」——同店品名原文完全穩定，原文精確比對就夠；跨店比價與品項層級
  分類維持 no-go。設計經一輪 grilling 定案（判定用「最近一次 vs 先前中位數」
  而非首末比、陳舊標旗標不濾除、三道清理各自留數字），細節見架構的 export.py
  與 web/query.html 條目。**開票前的量測有四個數字是錯的，實作時才現形**：
  ①可追蹤配對 127→**128**（票上那個是拿原始店名量的，而定案是用顯示名）
  ②漲價清單 10→**9**（10 是「套守衛之前」的數，被擋下的那 1 個不會出現在
  清單上——票上同時寫「清單 10 列」與「排除 1 個」本身就自相矛盾）
  ③同日不一致 11→**8**（量測腳本用 `if p:` 判斷，**折讓的負價漏了進來**，把
  「正價 vs 折讓」誤算成同日不一致；`p > 0` 才是對的）。
  ④payload 增量估 22 KB、實際 **+94 KB**（估的時候只算了緊湊陣列，實際帶
  每個價格點的日期/單價/發票號碼且照既有格式 indent=1）。教訓同一條：
  **拋棄式量測腳本的判斷式要和實作逐字對齊，不然基準數字會自己漂**。
  測試 69→71（偵測端的三道清理＋邊界＋回推、視圖端的清單/排除說明/n≥3 才畫圖/
  搜尋/跳發票清單）；惡意字串測試改成逐視圖各走一趟（原本只走預設的發票清單，
  新視圖的跳脫等於沒驗到）；golden 新增 query-price、既有四頁零差異
- ✅ #23 兩軸 review 補修（2026-08-02，issue #24）：五項全屬同一類——**文件或
  文案說了一件事、程式做另一件**。①三道清理只印了一道：`rowsDropped`／
  `daysDropped` 算完進了 payload 卻沒有任何頁面讀它，**對使用者而言仍是靜默
  丟棄**（README 那句「會說明各排除了幾筆」因此是假話）→ 全表底下補排除說明，
  措辭照實作寫（`rowsDropped` 含「沒有單價也回推不出」的列，不只贈品與折讓），
  fixture 的 stats 改成非零讓 golden 真的拍得到它。②`max(2, PRICE_RULE
  ["minPoints"])` 夾下限，而頁面文案直接印那個常數——設成 1 會頁面說「至少 1
  個」而行為是 2；夾制刪掉、常數成為唯一權威，下限 2 改由測試釘住。③#23 唯一
  點名的資料面坑（舊版 M/D CSV 的 `quantity`／`unit_price` 皆 None）沒有測試
  → 補一列 `price=None, qty=None`。④`_price_of` 的 docstring 指向它沒碰的欄位
  （寫資料庫欄名，實際讀 payload 的鍵）。⑤「已排除 N 項」只計本來會上榜的、
  全表標「散布 N 倍」的可能更多，兩個數字對不起來 → 文案講清楚。三個新守衛
  逐一還原確認變紅且訊息是人話。**駁回三項**（理由記在 #24，免得下輪再提）：
  連結改掛日期欄（發票號碼本身就是可點的線索）、`CHART_MIN_POINTS` 搬進
